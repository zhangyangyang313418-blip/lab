...
```text
outputs/mla-fee-export-template/MLA测试项目及费用预估_test-flow组别顺序模板.xlsx
```
...
```text
outputs/workbook-edits/final/MLA测试项目及费用预估_全部特殊项目标识_费用汇总补全_蓝色费用表格已填写_实验室页备注修正_电脑报告费用补全.xlsx
outputs/mla-fee-export-template/MLA测试项目及费用预估_test-flow组别顺序模板.xls.bak3
```

说明：

- `_电脑报告费用补全.xlsx` 是当前完整基底，保留 7 个 sheet、费用计算公式列、`费用预估` 右侧蓝色汇总工具区、`费用对比` 顶部工具区和 `费用规则校验`。
- `.bak3` 是旧 `.xls` 历史备份，只保留了早期样式和数据结构；由于它缺少公式工具区和校验页，不能再作为完整最终导出基准。
...
- Sheet 顺序：`费用预估`、`SGS`、`华测`、`苏勃`、`费用对比`、`特殊项目费用`、`费用规则校验`
...
当前这个命令已改为以完整 `.xlsx` 文件为基底，并直接写 `outputs/mla-fee-export-template/MLA测试项目及费用预估_test-flow组别顺序模板.xlsx`。后续不要再用旧 `.xls` 作为完整最终导出基底。
...
- `src/tests/mlaEnvironmentFeeTemplateExport.test.ts`：锁定最终 `.xlsx` 必须包含 7 个 sheet、费用计算公式列、蓝色费用工具区、`费用对比` 原生柱状图、隐藏图表数据源、`Computer Fee` / `Report Fee`。
...
- 不要把 `_电脑报告费用补全.xlsx` 删除，它是当前完整结构恢复锚点。
